...
@section('user-title', 'Blog Post Edit')
                ...
                <div class="p-0 m-auto col-md-11">
                    <div class="card">
                        ...
                        <div class="card-body bg-light">
                            <div class="row">
                                <div class="p-0 m-auto col-md-11">
                                    <form action="{{ route('bg_post.update', $post->id) }}" method="POST">
                                        @csrf
                                        @method('PATCH')
                                        <div class="mb-4 row">
                                            <label for="" class="col-sm-3 col-form-label">Title</label>
                                            <div class="col-sm-10">
                                                <input type="text" class="form-control" name="title" id="title" value="{{ $post->title }}" placeholder="Blog Category Name">
                                                @error('title')
                                        ...
                                        <div class="mb-4 row">
                                            ...
                                            <div class="col-sm-10">
                                                <textarea class="summernote form-control" id="summernote" name="post_content" cols="30" rows="10">{{ $post->content }}</textarea>
                                                @error('post_content')
                                                    <span class="text-danger">{{ $message }}</span>
                                                @enderror
                                            </div>
                                            ...
                                            <div class="col-sm-10">
                                                <select class="form-control" name="bg_category_id" id="bg_category_id">
                                                    ...
                                                    @foreach ($categories as $category)
                                                        <option value="{{ $category->id }}" {{ $post->bg_category_id == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                                                    @endforeach
                                                    @endif
                                                </select>
